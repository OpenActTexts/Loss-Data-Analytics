...
\]
Moreover, if the *pgf* exists on an interval $[0,s^\ast)$ with $s^\ast>1$, then the *mgf* $M_N(\cdot)$ exists on $(-\infty,\log(s^\ast))$, and hence uniquely specifies the distribution of $N$ by [Theorem 2.1]. (As a reminder, throughout this text we use *log* as the natural logarithm, not the base ten (common) logarithm or other version.) The following result for *pgf* is an analog of [Theorem 2.1], and in particular justifies its name. 
...
\]
In fact, more generally, using either a generalization of the above or using [Theorem 2.1], we see that
\[
...
`r HideExample('2.3.1', 'Show Example Solution')`
...
$$
\begin{array}{ll}
l(m,q) &= \sum_{i=1}^n \log\left(\binom{m}{x_i}\right) + \left({\sum_{i=1}^n x_i}\right)\log(q) \\
...
`r HideProofTheory('Binomial.1',"Show A Verification of Equation (2.2)")`
...
`r HideExample('2.5.1', 'Show Example Solution')`

**Solution.** For the Poisson distribution as a member of the (\(a,b\),0) class, we have \(a=0\) and \(b=\lambda=2\). Thus, we may use the recursion \(p_k = \lambda p_{k-1}/k= 2 p_{k-1}/k\) for each type, after determining starting probabilities. The calculation of probabilities for $k\leq 3$ is shown in [Table 2.2].
...
Appendix Chapter \@ref(C:AppB) provides additional background about this important expression.
...
`r HideExample('2.6.1', 'Show Example Solution')`
...
`r HideExample('2.6.2', 'Show Example Solution')`
