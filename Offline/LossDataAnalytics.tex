...
{\centering \includegraphics[width=0.5\linewidth]{LossDataAnalytics_files/figure-latex/unnamed-chunk-7-1} 
...
\[\begin{aligned}
    ...
    &=\mathrm{E} \left[ h(X,Y)^2\right] - \mathrm{E}\left(\left\{\mathrm{E} \left[ h(X,Y) | X \right] \right\}^2\right).\label{eq:AppBEV}
\end{aligned}\]
...
\(\mathrm{E}[h(X,Y)]\) and variance \[\begin{aligned}
    ...
    - \left\{\mathrm{E}[h(X,Y)]\right\}^2. \label{eq:AppBVE}
\end{aligned}\]

Thus, adding Equations \eqref{eq:AppBEV} and \eqref{eq:AppBVE} leads to the
