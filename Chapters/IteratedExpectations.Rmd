...
<h4 style="text-align: left;"><a id="EXM:S2a:LTV:display" href="javascript:toggleTheory('EXM:S2a:LTV','EXM:S2a:LTV:display');"><i><strong>Show Technical Detail</strong></i></a></h4>
...
$$\begin{aligned}
    ...
    &=\mathrm{E} \left[ h(X,Y)^2\right] - \mathrm{E}\left(\left\{\mathrm{E} \left[ h(X,Y) | X \right] \right\}^2\right).(\#eq:AppBEV)
\end{aligned}$$
...
$$\begin{aligned}
    ...
    - \left\{\mathrm{E}[h(X,Y)]\right\}^2. (\#eq:AppBVE)
\end{aligned}$$    

Thus, adding Equations \@ref(eq:AppBEV) and \@ref(eq:AppBVE) leads to the unconditional variance $\mathrm{Var} \left[ h(X,Y) \right]$.
