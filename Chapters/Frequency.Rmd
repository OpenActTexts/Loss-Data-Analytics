...
\end{cases}
\end{equation}
In the above, $\lfloor \cdot \rfloor$ denotes the floor function; $\lfloor x \rfloor$ denotes the greatest integer less than or equal to $x$. We note that the `r Gloss('survival function', '2.1')` of $N$, denoted by $S_N(\cdot)$, is defined as the ones'-complement of $F_N(\cdot)$, *i.e.* $S_N(\cdot):=1-F_N(\cdot)$. Clearly, the latter is another characterization of the distribution of $N$.
...
\]
The `r Gloss('pmf')` of $N$ can be derived from the *pgf* as follows:
\[
p_N(m)=\begin{cases} 
P_N(0), &m=0;\cr
&\cr
