...
From Appendix \@ref(C:AppA), the `r Gloss('MLE')` has some nice large-sample properties, under certain regularity conditions. We presented the results for a single parameter in Appendix \@ref(C:AppA), but results are true for the case when $\boldsymbol{\theta}$ contains multiple parameters. In particular, we have the following results, in a general case when $\boldsymbol{\theta}=(\theta_1,\theta_2,\cdots,\theta_p)$.
