...
\end{equation}
We note that $\mu_N$ is the expected value of the random variable $N$, *i.e.* $\mu_N=\mathbf{E}{N}$. This leads to a general class of measures, the **moments*8 of the distribution; the $r$-th moment of $N$, for $r> 0$,  is defined as $\mathbf{E}{N^r}$ and denoted by $\mu_N'(r)$. Hence, for $r>0$, we have  
\begin{equation}
...
Another basic aspect of a distribution is its dispersion, and of the various measures of dispersion studied in the literature, the **standard deviation** is the most popular. Towards defining it, we first define the **variance** of $N$, denoted by $\mathbf{Var}{N}$, as $\mathbf{Var}{N}:=\mathbf{E}{(N-\mu_N)^2}$ when $\mu_N$ is finite. By basic properties of the expected value of a random variable, we see that $\mathbf{Var}{N}:=\mathbf{E}{N^2}-(\mathbf{E}{N})^2$. The standard deviation of $N$, denoted by $\sigma_N$, is defined as the square-root of $\mathbf{Var}{N}$. Note that the latter is well-defined as $\mathbf{Var}{N}$, by its definition as the average squared deviation from the mean, is non-negative; $\mathbf{Var}{N}$ is denoted by $\sigma_N^2$. Note that these two measures take values in $[0,\infty)$.  
...
```{theorem, freq-thm1}
Let $N$ be a count random variable such that $\mathbf{E}{e^{t^\ast N}}$ is finite for some $t^\ast>0$. We have the following:
...
```{theorem, pgfthm}
Let $N$ be a count random variable such that $\mathbf{E}{(s^{\ast})^N}$ is finite for some $s^\ast>1$. We have the following:
...
\end{equation*}
The reason for its name is that the p.m.f. takes values among the terms that arise from the binomial expansion of $(q +(1-q))^m$. This realization then leads to the the following expression for the pgf of the binomial distribution:
\[
...
\]
Note that interestingly for the Poisson distribution $\mathbf{E}{N}=\mathbf{Var}{N}$. 
...
\end{eqnarray*}
We note that when $\beta>0$, we have $\mathbf{Var}{N} >\mathbf{E}{N}$. In other words, this distribution is **overdispersed** (relative to the Poisson); similarly, when $q>0$ the binomial distribution is said to be **underdispersed** (relative to the Poisson).
...
*Example: Zero Truncated/Modified Poisson*. Consider a Poisson distribution with parameter \(\lambda=2\). Calculate \(p_k, k=0,1,2,3\), for the usual (unmodified), truncated and a modified
...
after determining starting probabilities. The calculation of probabilities for $k\leq 3$ is shown in the table below.
...
*Exercise. Exam "C" 170*. In a certain town the number of common
...
*Example*. Suppose that \(N|\Lambda \sim\) Poisson\((\Lambda)\) and that \(\Lambda \sim\) gamma with mean of 1 and variance of 2. Determine the probability that \(N=1\).  
...
<!-- \caption{Singaporean Automobile Accident Data} -->
<!-- \label{tabsing} -->
$$\begin{matrix}
...
<!-- \caption{Comparison of Observed to Fitted Counts: Singaporean Auto Data} -->
<!-- \label{tabsing2} -->
$$\begin{matrix}
\begin{array}{|c|c|c|}
\hline
...
**Exercise 2.11.** (*Course 3: May 2000, Exercise 37.*) You are given:
...
**Exercise 2.12.** (*SOA Course C/Exam 4: May 2001, 19.*) During a one-year period, the number of accidents per day was distributed as follows:
