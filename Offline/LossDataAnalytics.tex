...
Recall from Chapter 1 that setting the price of an insurance good can be
a complex problem. In manufacturing, the cost of a good is (relatively)
known. In other financial service areas, market prices are available. In
insurance, we can generalize the price setting as follows: start with an
expected cost. Add ``margins'' to account for the product's riskiness,
expenses incurred in servicing the product, and a profit/surplus
allowance for the insurer.
...
three samples of size \(5\) given in Table 2.1 below. The data was
constructed to cover the three orderings of the sample mean and
variance. As show in the Figure \ref{fig:MLEab0}, and supported by
theory, if \(\hat{\mu}<\hat{\sigma}^2\) then the negative binomial will
result in a higher maximum likelihood value; if
...
probabilities for \(k\leq 3\) is shown in the table below.
...
\textbf{Example 2.6.1. ``Exam C'' 170}. In a certain town the number of
...
\text{Children} & 0.3 & 3\\\hline
\text{Adult Non-Smokers} & 0.6 & 1\\\hline
\text{Adult Smokers} & 0.1 & 4\\\hline
\end{array}
\end{matrix}\]

Table 2.3: The distribution of the population and the mean number of
colds
...
accidents per policyholder as given in Table 2.4.
...
\hline
\text{No. of Policies with k accidents }(m_k) & 6,996 & 455 & 28 & 4 & 0 & 7483\\
\hline
\end{array}
\end{matrix}\]
...
observed counts is given by Table 2.5 below, where \(\hat{p}_k\)
represents the estimated probabilities under the fitted Poisson
distribution.
...
Table 2.5: Comparison of Observed to Fitted Counts: Singaporean Auto
Data
...
\textbf{Exercise 2.11.} (\emph{Course 3: May 2000, Exercise 37.}) You
are given:
...
\textbf{Exercise 2.12.} (\emph{SOA Course C/Exam 4: May 2001, 19.})
...
\tightlist
\item
  \textbf{N.D. Shyamalkumar}, The University of Iowa, is the principal
  author of the initital version of this chapter. Email:
  \href{mailto:shyamal-kumar@uiowa.edu}{\nolinkurl{shyamal-kumar@uiowa.edu}}
  for chapter comments and suggested improvements.
\item
  \textbf{Krupa Viswanathan}, Temple University,
  \href{mailto:ksubrama@temple.edu}{\nolinkurl{ksubrama@temple.edu}},
  provided substantial improvements.
\end{itemize}
...
for \(x > 0\). For \(\alpha>0\),
\[\mu_{k}^{\prime} = \mathrm{E}\left( X^{k} \right) = \int_{0}^{\infty}{\frac{1}{\left( \alpha - 1 \right)!\theta^{\alpha}}x^{k + \alpha - 1}e^{- x / \theta} dx} = \frac{\Gamma\left( k + \alpha \right)}{\Gamma\left( \alpha \right)}\theta^{k}\]
...
and \(\mathrm{Var}\left( X \right) = \alpha\theta^{2}\).
...
and \(\mathrm{Var}\left( X \right) = \alpha\theta^{2} = 16\).
...
\[M_{X}\left( t \right) = \mathrm{E}\left( e^{\text{tX}} \right) = \int_{0}^{\infty}{e^{\text{tx}}be^{- bx} dx} = \int_{0}^{\infty}{be^{- x\left( b - t \right)} dx} = \frac{b}{\left( b - t \right)}.\]
...
\textbf{Example 3.1.4. SOA Exam Question.} Let \(X_{1}\), \(X_{2}\), .,
\(X_{n}\) be independent \(\text{Ga}\left( \alpha_{i},\theta \right)\)
...
\[M_{S}\left( t \right) = \mathrm{E}\left( e^{\text{tS}} \right) = \mathrm{E}\left( e^{t\sum_{i = 1}^{n}X_{i}} \right) \\
= \mathrm{E}\left( \prod_{i = 1}^{n}e^{tX_{i}} \right) = \prod_{i = 1}^{n}{\mathrm{E}\left( e^{tX_{i}} \right) = \prod_{i = 1}^{n}{M_{X_{i}}\left( t \right)}} .\]
...
\(\mathrm{E}\left( S \right) = \left. \ \frac{\partial M_{S}\left( t \right)}{\partial t} \right|_{t = 0} = \alpha\theta\)
where \(\alpha = \sum_{i = 1}^{n}\alpha_{i}\), and
\[\mathrm{E}\left( S^{2} \right) = \left. \ \frac{\partial^{2}M_{S}\left( t \right)}{\partial t^{2}} \right|_{t = 0} = \left( \alpha + 1 \right)\alpha\theta^{2}.\]
...
\textbf{Example 3.3.5. SOA Exam Question.} The distribution of the
random variable \(X\) is an equally weighted mixture of two Poisson
distributions with parameters \(\lambda_{1}\) and \(\lambda_{2}\)
respectively. The mean and variance of \(X\) are 4 and 13, respectively.
Determine \(\Pr\left( X > 2 \right)\).
...
\[\mathrm{E}\left( X^{2} \right) = 0.5\left( \lambda_{1} + \lambda_{1}^{2} \right) + 0.5\left( \lambda_{2} + \lambda_{2}^{2} \right) = 13 + 16\]
...
\textbf{Example 3.3.7. SOA Exam Question.} \(X\) has a binomial
distribution with a mean of \(100q\) and a variance of
\(100q\left( 1 - q \right)\) and \(q\) has a beta distribution with
parameters \(a = 3\) and \(b = 2\). Find the unconditional mean and
variance of \(X\).
...
\(\mathrm{E}\left( X \right) = \mathrm{E}\left\lbrack \mathrm{E}\left( X\left| q \right.\  \right) \right\rbrack = \mathrm{E}\left( 100q \right) = 100\mathrm{E}\left( q \right) = 60\),

\[\mathrm{Var}\left( X \right) = \mathrm{E}\left\lbrack \mathrm{Var}\left( X\left| q \right.\  \right) \right\rbrack + \mathrm{Var}\left\lbrack \mathrm{E}\left( X\left| q \right.\  \right) \right\rbrack = \mathrm{E}\left\lbrack 100q\left( 1 - q \right) \right\rbrack + \mathrm{Var}\left( 100q \right)\]

\(= 100\mathrm{E}\left( q \right) - 100\mathrm{E}\left( q^{2} \right) + 100^{2}V\left( q \right) = 420\).

\begin{center}\rule{0.5\linewidth}{\linethickness}\end{center}

\textbf{Example 3.3.8. SOA Exam Question.} Claim sizes, \(X\), are
uniform on for each policyholder. varies by policyholder according to an
...
\[\mathrm{E}\left( Y^{L} \right) = \int_{100}^{\infty}\left( x - 100 \right)f_{X}\left( x \right)dx \\
= {\int_{100}^{\infty}{S_{X}\left( x \right)}dx = 1000e}^{- \frac{100}{1000}},\]
and
\[\mathrm{E}\left\lbrack \left( Y^{L} \right)^{2} \right\rbrack = \int_{100}^{\infty}\left( x - 100 \right)^{2}f_{X}\left( x \right)dx \\
...
same mean. Hence, \(\mathrm{E}\left( Y^{P} \right)\)=1000 and
\[\mathrm{E}\left\lbrack \left( Y^{P} \right)^{2} \right\rbrack = 2 \times 1000^{2}.\]
Using the relationship between \(Y^{L}\) and \(Y^{P}\) we find
\[\mathrm{E}\left( Y^{L} \right) = \ \mathrm{E}\left( Y^{P} \right)S_{X}\left( 100 \right){= 1000e}^{- \frac{100}{1000}}\]

\[\mathrm{E}\left\lbrack \left( Y^{L} \right)^{2} \right\rbrack = \mathrm{E}\left\lbrack \left( Y^{P} \right)^{2} \right\rbrack S_{X}\left( 100 \right) = 2 \times 1000^{2}e^{- \frac{100}{1000}}.\]
...
\(\mathrm{E}\left( Y^{P} \right) = \frac{\int_{4}^{10}\left( x - 4 \right)0.02xdx}{{1 - F}_{X}\left( 4 \right)} = \frac{2.88}{0.84} = 3.43\).
...
\[\frac{\mathrm{E}\left( X \right) - \mathrm{E}\left( Y^{L} \right)}{\mathrm{E}\left( X \right)} = \frac{\theta - \theta e^{- d / \theta}}{\theta} = 1 - e^{- d / \theta} = 0.7.\]
Then, \(e^{- d / \theta} = 0.3\).
...
\hypertarget{toggleExampleLoss.4.4}{}
\textbf{Solution.}
...
\(\mathrm{E}\left( Y \right) = \mathrm{E}\left( X \land 1 \right) = \int_{0}^{1}\frac{x^{2}(4 - x)}{9}dx + \int_{1}^{3}\frac{x\left( 4 - x \right)}{9}dx = 0.935\).
...
\[\mathrm{E}\left( Y_{2006}^{L} \right) = \mathrm{E}\left( X \land 600 \right) - \mathrm{E}\left( X \land 100 \right) = 1000\left( {1 - e}^{- \frac{600}{1000}} \right) - 1000\left( {1 - e}^{- \frac{100}{1000}} \right)\]

\(= 356.026\).

\[\mathrm{E}\left( Y_{2007}^{L} \right) = 1.05\left\lbrack \mathrm{E}\left( X \land 571.43 \right) - \mathrm{E}\left( X \land 95.24 \right) \right\rbrack\]

\(= 1.05\left\lbrack 1000\left( {1 - e}^{- \frac{571.43}{1000}} \right) - 1000\left( {1 - e}^{- \frac{95.24}{1000}} \right) \right\rbrack\)

\(\mathbf{=}361.659\).

\(\mathrm{E}\left( Y_{2006}^{P} \right) = \frac{356.026}{e^{- \frac{100}{1000}} = 393.469}\).

\(\mathrm{E}\left( Y_{2007}^{P} \right) = \frac{361.659}{e^{- \frac{95.24}{1000}} = 397.797}\).
...
insurance premium. Under that policy, when the loss exceeds the
deductible, the insurer is not required to pay until the insured has
paid the fixed deductible. We now introduce reinsurance, a mechanism of
insurance for insurance companies. Reinsurance is a contractual
arrangement under which an insurer transfers part of the underlying
insured risk by securing coverage from another insurer (referred to as a
reinsurer) in return for a reinsurance premium. Although reinsurance
involves a relationship between three parties: the original insured, the
insurer (often referred to as cedent or cedant) and the reinsurer, the
parties of the reinsurance agreement are only the primary insurer and
the reinsurer. There is no contractual agreement between the original
...
of \(\hat{\theta}\)
\(\widehat{Var}\left( \hat{\theta} \right) = \left. \ \left\lbrack \mathrm{E}\left( \frac{d^{2}\ln L \left( \theta \right)}{d\theta^{2}} \right) \right\rbrack^{- 1} \right|_{\theta = \hat{\theta}} = \frac{{\hat{\theta}}^{2}}{4} = 28,446,222\).
...
\(\mathrm{E}\left( \frac{\partial^{2}\text{lnL}\left( \mu,\sigma \right)}{\partial\sigma^{2}} \right) = \frac{6}{\sigma^{2}} - \frac{3}{\sigma^{4}}\sum_{i = 1}^{6}{\mathrm{E}\left( \ln x_{i} - \mu \right)}^{2} = \frac{6}{\sigma^{2}} - \frac{3}{\sigma^{4}}\sum_{i = 1}^{6}{V\left( \ln x_{i} \right) = \frac{6}{\sigma^{2}} - \frac{3}{\sigma^{4}}\sum_{i = 1}^{6}{\sigma^{2} = \frac{- 12}{\sigma^{2}}}}\).
...
The limited policy introduced in Section \ref{S:PolicyLimits} is an
...
\textbf{Example 3.5.4. SOA Exam Question.} The random variable has
survival function:
...
specific parametric distribution. Section \ref{S:NonParInf} describes
...
\ref{S:ModelSelection}. Although our focus is on continuous data, the
...
\ref{S:ModifiedData} describes estimation for alternative sampling
...
with Section \ref{S:BayesInference} on Bayesian inference, an
...
\textbf{Example 4.1.5. Exam C Question 3.} You study five lives to
...
\textbf{Example 4.1.6. Exam C Question 59.} The graph below shows a
...
\textbf{Exaxmple 4.1.7. Exam C Question 40 (modified).} A sample of
claim payments is:
...
\textbf{Exercise 4.1.10. Exam C Question 1.} You are given:
...
\section{Model Selection}\label{S:ModelSelection}
...
\subsection{Summarizing Model Selection Based on a Training
Dataset}\label{summarizing-model-selection-based-on-a-training-dataset}
...
a specific parametric form, as summarized in Section \ref{S:NonParInf}.
...
Generally, models are not proper subsets of one another so overall
goodness of fit statistics, summarized in Section
@ref\{S:NonparametricModelSelection\}, are useful for model comparison.
For discrete data, a \textbf{chi-square goodness of fit statistic} is
...
\section{Estimation using Modified Data}\label{S:ModifiedData}
...
\textbf{Example 4.3.1. Exam C Question 44.} You are given:
...
\textbf{Example 4.3.2. Exam C Question 152.} You are given:
...
\textbf{Example 4.3.3. Course 4: Fall 2000, Question 22.} You are given
the following information about a random sample:
...
\textbf{Example 4.3.4. Exam C Question 195.} You are given the following
...
\textbf{Example 4.3.5. Exam C Question 252.} The following is a sample
of 10 payments:
...
\textbf{Example 4.3.6. Exam C Question 135.} For observation \(i\) of a
survival study:
...
\textbf{Exercise 4.3.7. Exam C Question 252. - Continued.}
...
\textbf{Exercise 4.4.1. Exam C Question 157.} You are given:
...
\textbf{Example 4.4.2. Exam C Question 43.} You are given:
...
\textbf{Example 4.4.3. Exam C Question 190.} For a particular policy,
the conditional probability of the annual number of claims given
...
\textbf{Example 4.4.4. Exam C Question 11.} You are given:
...
\textbf{Exercise 4.4.5. Exam C Question 15.} You are given:
...
\textbf{Example 4.4.6. Exam C Question 29.} You are given:
...
\textbf{Example 4.4.7. Exam C Question 215.} You are given:
...
Section \ref{S:RCIntroduction} we explain why insurers need to
...
introduce Section \ref{S:PoissonRegression} the Poisson regression as a
pricing tool to achieve such premium differentials. The concept of
...
\section{Introduction}\label{S:RCIntroduction}
...
\section{Poisson Regression Model}\label{S:PoissonRegression}
...
\subsection{Need for Poisson Regression}\label{S:Need.Poi.reg}
...
consider the data separately for each smoking status. In Table 8.1 we
present the data for this portfolio.
...
and \(\mathrm{E~}{(Y)}=\mathrm{Var~}{(Y)}=\mu\). Furthermore, the mle of
...
\(\mu_{(2)}\) (non-smoker), we see from Table 8.1 that
\(\hat{\mu}_{(1)}=0.0926\) and \(\hat{\mu}_{(2)}=0.0746\). This simple
example shows the basic idea of risk classification. Depending on the
smoking status a policyholder will have a different risk characteristic
and it can be incorporated through varying Poisson parameter in
computing the fair premium. In this example the ratio of expected loss
frequencies is \(\hat{\mu}_{(1)}/\hat{\mu}_{(2)}=1.2402\), implying that
smokers tend to visit doctor's office 24.02\(\%\) times more frequently
...
premium) \(\hat{\mu}\) is 0.0792, obtained from the last column of Table
8.1. It is easily verified that
...
Poisson pmf in terms of the \emph{rate} parameter \(\lambda\), based on
...
\def\labelenumi{\arabic{enumi}.}
\item
  Regarding Table 8.1 answer the followings.
...
such as the one shown in Table 8.2, collected from all policyholders.
Clearly there are \(2 \times 3=6\) different risk classes in total. Each
row of the table shows a combination of different risk characteristics
of individual policyholders. Our goal is to compute six different
premiums for each of these combinations. Once the premium for each row
has been determined using the given exposure and claim counts, the
insurer can replace the last two columns in Table 8.2 with a single
...
\emph{tariff cell}; thus, in Table 8.2 the number of tariff cells is
six, same as the number of risk classes.
...
Let us now look at the loss information in Table 8.2 more closely. The
exposure in each row represents the sum of the length of insurance
coverages, or in-force times, in the unit of year, of all the
policyholders in that tariff cell. Similarly the claim counts in each
row is the number of claims at each cell. Naturally the exposures and
claim counts vary due to the different number of drivers across the
cells, as well as different in-force time periods among the drivers
within each cell.
...
fully exploit the dataset, we will construct a pricing model from Table
8.2 using the Poisson regression, for the remaining part of the chapter.
...
regression framework. The developments here is based on Table 8.2.
Recall that the loss count of a policyholder is described by the Poisson
regression model with rate \(\lambda\) and the exposure \(m\), so that
the expected loss count becomes \(m\lambda\). As \(m\) is a known
constant, we are essentially concerned with modelling \(\lambda\), so
that it responds to the change in the rating factors. Among other
possible functional forms, we commonly choose the
multiplicative\footnote{Preferring the multiplicative form to others
  (e.g., additive one) was already hinted in \eqref{eq:log-lin-mu}.}
relation to model the Poisson rate \(\lambda_{jk}\) for rating factor
(\(j,k\)):
...
mle \(b_i\) using the method in the technical supplement at the end of
this chapter (Section \ref{S:mle-Pois-reg}).

\subsection{Numerical Examples}\label{numerical-examples}

We present two numerical examples of the Poisson regression. In the
first example we construct a Poisson regression model from Table 8.2,
which is a dataset of a hypothetical auto insurer. The second example
uses an actual industry dataset with more risk factors. As our purpose
is to show how the Poisson regression model can be used under a given
classification rule, we are not concerned with the quality of the
Poisson model fit in this chapter.
...
auto insurer with two risk factors, as given in Table 8.2. We now apply
the Poisson regression model to this dataset. As done before, we have
set \((j,k)=(1,1)\) as the base tariff cell, so that
\(f_{11}=f_{21}=1\). The result of the regression gives the coefficient
estimates \((b_0, b_1,b_2,b_3)=(-2.3359, -0.3004, -0.7837, -1.0655 )\),
which in turn produces the corresponding relativities
...
The fitting result is given in Table 8.3, for which we have several
comments.
...
\end{array}
\end{matrix}\] {[}Table 8.3{]} : Singapore insurance claims data

Let us try several examples based on Table 8.3. Suppose a male
policyholder aged 40 who owns a 7-year-old vehicle of type A. The
expected claim frequency for this policyholder is then given by
...
To obtain the mle of \(\beta=(\beta_0, \ldots, \beta_k)^{\prime}\), we
...
Numerically solving this equation system gives the mle of \(\beta\),
denoted by \(\mathbf{ b}=(b_0, b_1, \ldots, b_k)^{\prime}\). Note that,
as \(\mathbf{ x}_i=(1, x_{i1}, \ldots, x_{ik})^{\prime}\) is a column
vector, equation \eqref{eq:score-ft-Poi} is a system of \(k+1\) equations
with both sides written as column vectors of size \(k+1\). If we denote
\(\hat{\mu}_i=m_i \exp(\mathbf{ x}^{\prime}_i \mathbf{ b})\), we can
rewrite \eqref{eq:score-ft-Poi} as
...
example, Table 8.2 provides collective loss information for each risk
class after grouping individual policies. When this is the case, \(y_i\)
and \(m_i\), the quantities needed for the mle calculation in
\eqref{eq:score-ft-Poi}, are unavailable for each \(i\). However this does
not pose a problem as long as we have the total loss counts and total
exposure for each risk class.
...
exposure, we see that for the Poisson regression the mle \(\mathbf{ b}\)
is the same whether if we use the individual data or the grouped data.

\textbf{Information matrix}\\
Taking second derivatives to \eqref{eq:ll-Poi-reg} gives the information
matrix of the mle estimators,
...
estimate the relevant variances and covariances of the mle
...
\tightlist
\item
  \textbf{Joseph H. T. Kim}, Yonsei University
\end{itemize}
...
that \(X_1,\ldots, X_n, X_{n+1}\) are iid.
...
Random variables \(X_j\) are assumed to be iid for \(j=1,\ldots,n\). The
...
independent of \(N\) and are iid with Pareto distribution
...
\item
  ...
  the observations are iid, it follows that
...
independent of \(N\) and are iid with exponential distribution
...
to be iid. If the subscripts indicate year 1, year 2 and so on up to
year \(n\), then the iid assumption means that the policyholder has the
same exposure to loss every year. For commercial insurance this
...
to year. The annual losses for the fleet are not iid.
...
random variables \(Y_{jk}\) are iid across all vehicles and years for
the policyholder. With this assumption the means
...
\item
  ...
  population with pdf \(f(\lambda)=6\lambda(1-\lambda)\) with
  \(0<\lambda<1\).
\end{itemize}
...
Chapter four includes a section on Bayesian inference and it is assumed
that the reader is familiar with that material. This section will
compare Bayesian inference and Bühlmann credibility and show connections
...
is multiplied times \(\lambda\) in the exponential of the gamma pdf
whereas in chapter four \(\lambda\) is divided by parameter \(\theta\).
...
\(j=1,\ldots, n\). Random variables \(Y_j\) are iid with Pr\([Y=1|p]=p\)
and Pr\([Y=0|p]=1-p\) The number of heads in \(n\) tosses can be
represented by the random variable \(X=Y_1+\cdots+Y_n\). We want to
estimate \(p=E[Y_j]\) using Bühlmann credibility:
...
\(X_{i1},\ldots, X_{in}\). The losses for a risk are iid across periods
as assumed in the Bühlmann model. For risk \(i\) the sample mean is
\(\bar{X}_i=\sum_{j=1}^{n}X_{ij}/n\) and the unbiased sample process
...
\(Y_{ik}\) are iid with mean E(\(Y_{ik}\))=\(\mu(\theta_i)\) and process
variance Var(\(Y_{ik}\))=\(\sigma^2(\theta_i)\). It follows that
...
\tightlist
\item
  \textbf{Gary Dean}, Ball State University is the author of the
  initital version of this chapter. Email:
...
\textbf{Example 10.1.} Let \(X\sim Gamma(\alpha,\theta)\), with
\(\alpha>0\) and \(\theta>0\), then for all \(k>0\),
...
\textbf{Example 10.2.} Let \(X\sim Weibull(\theta,\tau)\), with
\(\theta>0\) and \(\tau>0\), then for all \(k>0\),
...
\textbf{Example 10.3.} Let \(X\sim Pareto(\alpha,\theta)\), with
\(\alpha>0\) and \(\theta>0\), then for \(k>0\)
...
academic actuaries. However, for some \emph{rv}'s such as the log normal
...
\tightlist
\item
  \emph{\(X\) has a heavier right tail than \(Y\) if
  \(\gamma=\infty\);}\\
\item
  \emph{\(X\) and \(Y\) are proportionally equivalent in the right tail,
  if \(\gamma =c\in \mathbf{R}_+\);}
\item
  \emph{\(X\) has a lighter right tail than \(Y\) if \(\gamma=0\).}
\end{itemize}

\textbf{Example 10.4.} Let \(X\sim Pareto(\alpha, \theta)\) and
...
\textbf{Example 10.5.} Let \(X\sim Pareto(\alpha, \theta)\) and
...
Speaking bluntly, the \emph{VaR} measure outputs the smallest value of
\(X\) such that the associated \emph{cdf} first excesses or equates to
\(q\). In the fields of probability and statistics, the \emph{VaR} is
also known as the percentiles.

Here is how we should interpret \emph{VaR} in the lingo of actuarial
mathematics. \emph{VaR} is a forecast of the `maximal' probable loss for
a insurance product/portfolio or a risky investment occurring
...
\textbf{Example 10.6.} Consider an insurance loss \emph{rv}
\(X\sim Exp(\theta)\) for \(\theta>0\), then the \emph{cdf}~of \(X\) is
given by \[
F_X(x)=1-e^{-x/\theta}, \text{ for } x>0.
\] Since exponential distribution is a continuous distribution, the
...
\textbf{Example 10.7.} Consider an insurance loss \emph{rv}
\(X\sim Normal(\mu,\sigma^2)\) with \(\mu\in \mathbf{R}\) and
\(\sigma>0\). In this case, one may interpret the negative values of
\(X\) as profit or revenue. Because normal distribution is a continuous
distribution, the \emph{VaR} of \(X\) must satisfy
...
\textbf{Example 10.8.} Consider an insurance loss
\emph{rv}~\(Y\sim Log-Normal(\mu,\sigma^2)\), for \(\mu\in \mathbf{R}\)
and \(\sigma>0\). That is \(\log Y\sim Normal(\mu,\sigma^2)\), or
equivalently let \(X\sim Normal(\mu,\sigma^2)\), then
\(Y\overset{d}{=}e^{X}\) which is strictly increasing transformation.
Here, the notation `\(\overset{d}{=}\)' means equality in distribution.
The \emph{VaR} of \(Y\) is thus given by the exponential transformation
of the \emph{VaR} of \(X\). Precisely, for \(q\in [0,1]\), \[
...
\textbf{Example 10.9.} Consider an insurance loss \emph{rv} with the
...
F_X(x)=\left\{
         ...
         \end{array}
       \right.
\] By the definition of \emph{VaR}, we thus have, for example,
...
\textbf{Example 10.10.} Consider two loss \emph{rv}'s
...
\textbf{Example 10.11.} Consider an insurance loss
\emph{rv}~\(X\sim Normal (\mu,\sigma^2)\) with \(\mu\in \mathbf{R}\) and
\(\sigma>0\). Denoted by \(Z\) the standard normal \emph{rv}, for
\(q\in[0,1]\), the \emph{TVaR} of \(X\) can be computed via
...
We mentioned earlier in the previous subsection that the VaR of a
...
original \emph{rv} This is due to the linearity property of expectation.
However, the aforementioned finding can not be extended to non-linear
functions. The following example of log normal \emph{rv} serves as a
counter example.

\textbf{Example 10.12.} Consider an insurance loss
\emph{rv}~\(X\sim Log-Normal (\mu,\sigma^2)\), with
\(\mu\in \mathbf{R}\) and \(\sigma>0\). Recall that the p.d.f.~of log
normal distribution is formulated as \[
...
Evoking the formula of VaR for log normal \emph{rv}~reported in Example
...
Clearly, the \emph{TVaR} of log normal \emph{rv}~is not the exponential
...
\textbf{Example 10.13.} Consider an insurance loss
\emph{rv}~\(X\sim Exp(\theta)\) for \(\theta>0\), we have seen from the
...
In the SOA exam, we may use the expectation formulas provided in the
distribution table to compute \emph{TVaR}. Specifically, we have
...
excess loss function, and for many commonly used parametric
distributions, the formulas for calculating \(\mathrm{E}[X]\) and
\(\mathrm{E}[X\wedge\pi_q]\) can be found in the distribution table.

\textbf{Example 10.14.} Consider a loss
\emph{rv}~\(X\sim Pareto(\theta,\alpha)\) with \(\theta>0\) and
\(\alpha>0\). The \emph{cdf}~of \(X\) is given by \[
...
Via change of variable, we can also rewrite Equation \eqref{eq:cte-pdf} as
...
model selection, we refer you to \emph{Chapter 4 }.
...
\caption{\label{fig:Hist2}Left: Histogram of Transformed Loss.
Right:Histogram of normal scores of Loss}
\end{figure}
...
to Frank's copula with Maximum likelihood method.
...
literature. Two main families of the copula families are Archimedian
