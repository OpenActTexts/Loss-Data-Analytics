...
### Concentration Curve and a Gini Statistic 
We now introduce a modification of the classic Lorenz curve and Gini statistic that is useful in insurance applications. Specifically, we introduce a **concentration curve** which is a graph of the distribution of losses versus premiums, where both losses and premiums are ordered by premiums. To make the ideas concrete, we provide some notation and consider $i=1, \ldots, n$ policies. For the $i$th policy, let
...
The set of information used to calculate the concentration curve for the $i$th policy is $(y_i, P_i)$. 
...
where $\mathrm{I}(\cdot)$ is the indicator function, returning a 1 if the event is true and zero otherwise. The graph $\left(\hat{F}_P(s),\hat{F}_{L}(s) \right)$ is a **concentration curve**.
...
Figure \@ref(fig:LorenzVsOrdered) compares the Lorenz to the concentration curve. The left-hand panel shows the Lorenz curve. The horizontal axis is the cumulative proportion of policyholders (0, 0.2, 0.4, and so forth) and the vertical axis is the cumulative proportion of losses (0, 2/36, 7/36, and so forth). For the Lorenz curve, you first order by the loss size (which turns out to be the same order as premiums for this simple dataset). This figure shows a large separation between the distributions of losses and policyholders.

The right-hand panel shows the concentration  curve. Because observations are sorted by premiums, the first point after the origin (reading from left to right) is (2/34, 2/36). The second point is (6/34, 7/36), with the pattern continuing. From the figure, we see that there is little separation between losses and premiums.
...
```{r, LorenzVsOrdered, fig.cap='Lorenz versus Concentration Curve', out.width='90%', fig.asp=0.5, fig.align='center', echo=FALSE}
# EXAMPLE
P = c(2,4,5,7,16)
y = c(2,5,6,6,17)
n1 = length(y)
...
xlab="Premium Distn",ylab="", main="Concentration");abline(0,1)
mtext("Loss Distn", side=2, line=-0.5, at=1.1,   las=1, cex=1.0)
...
The concentration curve can be helpful to the analyst who thinks about forming profitable portfolios for the insurer. For example, suppose that $s$ is chosen to represent the 95*th* percentile of the premium distribution. Then, the horizontal axis, $\hat{F}_P(s)$, represents the fraction of premiums for this portfolio and the vertical axis, $\hat{F}_L(s)$, the fraction of losses for this portfolio. When developing premium principles, analysts wish to avoid unprofitable situations and make profits, or at least break even.

The expectation of the numerator of equation \@ref(eq:EmpLossDF) is $\sum_{i=1}^n \mathrm{E}~ y_i=\sum_{i=1}^n \mu_i$. Thus, if the premium principle is chosen such that $P_i= \mu_i$, then we anticipate a close relation between the premium and loss distributions, resulting in a 45 degree line. The 45 degree line is presents equality between losses and premiums, a break-even situation which is the benchmark for insurance pricing.

#### Gini Statistic {-}

The classic Lorenz curve shows the proportion of policyholders on the horizontal axis and the loss distribution function on the vertical axis. The concentration curve extends the classical Lorenz curve in two ways, (1) through the ordering of risks and prices by prices and (2) by allowing prices to vary by observation. We summarize the concentration curve in the same way as the classic Lorenz curve using a Gini statistic, defined as twice the area between the curve and a 45 degree line. The analyst seeks ordered concentration curves that approach passing through the 45 degree line; these have least separation between the loss and premium distributions and therefore small Gini statistics.


Specifically, the Gini statistic can be calculated as follows. Suppose that the empirical concentration curve is given by $\{ (a_0=0, b_0=0), (a_1, b_1), \ldots,$ $(a_n=1, b_n=1) \}$ for a sample of $n$ observations. Here, we use $a_j = \hat{F}_P(P_j)$ and $b_j = \hat{F}_{L}(P_j)$. Then, the empirical Gini statistic is
\begin{eqnarray}
\widehat{Gini} 
...
<h5 style="text-align: center;"><a id="displayAreaDemo" href="javascript:toggleEX('toggleAreaDemo','displayAreaDemo');"><i><strong>Show Gini Formula Details</strong></i></a> </h5><div id="toggleAreaDemo" style="display: none">
...
**Example -- Loss Distribution: Continued.** The Gini statistic for the Lorenz curve (left-hand panel of Figure \@ref(fig:LorenzVsOrdered)) is `r round(100*GiniLoss,digits=1)` percent. In contrast, the Gini statistic for concentration curve (right-hand panel) is `r round(100*Gini,digits=1)` percent. 


### Out-of-Sample Validation 

The benefits of out-of-sample validation for model selection were introduced in Section 4.2. We now demonstrate the use of the a Gini statistic and concentration curve in this context. The procedure follows:
...
* To compute base premiums, the analyst assumes a scale parameter that is common to all states that is to be estimated from the data. You can think of this common premium as based on a *community rating* principle. 
...
An out of sample validation set of `r NOutsamp` losses from each state is available. For each of the two rating procedures, determine the concentration curve and the corresponding Gini statistic. Choose the rate procedure with the lower Gini statistic.
...
Out of sample claims were generated from the same gamma distribution as the in-sample model, with `r NOutsamp` observations for each state. The following `R` code shows how to calculate the concentration curves.
...
```{r, fig.cap='', out.width='90%', fig.align='center', echo=TRUE} 
y <- Outy 
P1 <- Flatpred
P2 <- Predvec
n1 = length(y)
...
For these data, the Gini statistics are `r round(100*Gini1,digits=1)` percent for the flat rate premium and `r round(100*Gini2,digits=3)` percent for the state-specific alternative.  This suggests that the state-specific alternative procedure is strongly preferred to the base community rating procedure.
...
The value of the concentration curves and Gini statistics have been recently advanced in the paper of @denuit2019concentrationGini.
Properties of an extended version, dealing with relatives for new premiums, were developed by [@frees2011summarizing] and [@frees2014insurance]. In these articles you can find formulas for the standard errors and other additional background information.
