...
*Chapter Preview.* This chapter motivates the use of risk classification in insurance pricing and introduces readers to the Poisson regression as a prominent example of risk classification. In Section \@ref(S:RCIntroduction) we explain why insurers need to incorporate various risk characteristics, or rating factors, of individual policyholders in pricing insurance contracts. We then introduce Section \@ref(S:PoissonRegression) the Poisson regression as a pricing tool to achieve such premium differentials. The concept of exposure is also introduced in this section. As most rating factors are categorical, we show in Section \@ref(S:CatVarMultiTarriff) how the multiplicative tariff model can be incorporated in the Poisson regression model in practice, along with numerical examples for illustration.


## Introduction {#S:RCIntroduction}
...
## Poisson Regression Model {#S:PoissonRegression}
...
[Table `r chapnum`.1] : Number of visits to doctor's office in last year
...
### Exercises
<!-- %=================== -->
1. Regarding Table `r chapnum`.1 answer the followings.  
...
The fitting result is given in Table `r chapnum`.3, for which we have several comments.  
...
\end{array}
\end{matrix}$$
[Table `r chapnum`.3] : Singapore insurance claims data

Let us try several examples based on Table `r chapnum`.3. Suppose a male policyholder aged 40 who owns a 7-year-old vehicle of type A. The expected claim frequency for this policyholder is then given by 
...
## Technical Supplement -- Estimating Poisson Regression Models {#S:mle-Pois-reg}  
...
#### Contributor {-}
- **Joseph H. T. Kim**, Yonsei University
