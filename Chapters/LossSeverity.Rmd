...
Given $\Gamma\left( r + 1 \right) = r\Gamma\left( r \right)$, then $\mu_{1}^{\prime} = \mathrm{E}\left( X \right) = \alpha\theta$, $\mu_{2}^{\prime} = \mathrm{E}\left( X^{2} \right) = \left( \alpha + 1 \right)\alpha\theta^{2}$, $\mu_{3}^{\prime} = \mathrm{E}\left( X^{3} \right) = \left( \alpha + 2 \right)\left( \alpha + 1 \right)\alpha\theta^{3}$, and $\mathrm{Var}\left( X \right) = \alpha\theta^{2}$.
...
Hence, $\alpha = 4$. Since, $\mathrm{E}\left( X \right) = \alpha\theta = 8$, then $\theta = 2$ and $\mathrm{Var}\left( X \right) = \alpha\theta^{2} = 16$.
...
By finding the first and second derivatives of $M_{S}\left( t \right)$ at zero, we can show that $\mathrm{E}\left( S \right) = \left. \ \frac{\partial M_{S}\left( t \right)}{\partial t} \right|_{t = 0} = \alpha\theta$ where $\alpha = \sum_{i = 1}^{n}\alpha_{i}$, and
...
The solution can be simplified if we make use of the relationship between $X$ and $Y^{P}$. If $X$ is exponentially distributed with mean 1000, then $Y^{P}$ is also exponentially distributed with the same mean. Hence, $\mathrm{E}\left( Y^{P} \right)$=1000 and
