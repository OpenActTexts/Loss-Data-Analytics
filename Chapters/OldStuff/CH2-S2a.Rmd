# Technical Supplement: Iterated Expectations {#C:TS2a}

This supplement introduces the laws related to iterated expectations. In particular, Section \@ref(S:TS2a:CD) introduces the concepts of conditional distribution and conditional expectation. Section \@ref(S:TS2a:IE) introduces the Law of Iterated Expectations and the Law of Total Variance.
...
## Conditional Distribution and Conditional Expectation {#S:TS2a:CD}
...
## Iterated Expectations and Total Variance {#S:TS2a:IE}
...
*** 
<h4 style="text-align: left;"><a id="EXM:S2a:LTV:display" href="javascript:toggleDetail('EXM:S2a:LTV','EXM:S2a:LTV:display');"><i><strong>Show Technical Detail</strong></i></a></h4>
...
$$\begin{aligned}
    ...
    &=\mathrm{E} \left[ h(X,Y)^2\right] - \mathrm{E}\left(\left\{\mathrm{E} \left[ h(X,Y) | X \right] \right\}^2\right).(\#eq:TS2aEV)
\end{aligned}$$
...
$$\begin{aligned}
    ...
    - \left\{\mathrm{E}[h(X,Y)]\right\}^2. (\#eq:TS2aVE)
\end{aligned}$$    

Thus, adding Equations \@ref(eq:TS2aEV) and \@ref(eq:TS2aVE) leads to the unconditional variance $\mathrm{Var} \left[ h(X,Y) \right]$.
