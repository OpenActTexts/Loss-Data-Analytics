...
where $w$ is a probability density function centered about 0. Note that equation \@ref(eq:KDF) is a special case of the kernel density estimator where $w(x) = \frac{1}{2}I(-1 < x \le 1)$, also known as the *uniform kernel*. Other popular choices are shown in [Table 4.1].
...
To see how this works, as a special case of *g* we consider the loss per payment random variable is $Y = (X-d)_+$ and the *loss elimination ratio* introduced in Section 3.4.1. We can express this as
$$
LER(d) = \frac{\mathrm{E~}[X - (X-d)_+]}{\mathrm{E~}[X]} =\frac{\mathrm{E~}[\min(X,d)]}{\mathrm{E~}[X]} ,
$$
for a fixed deductible $d$. 

**Example. `r chapnum`.1.6. Bodily Injury Claims and Loss Elimination Ratios** 
...
**Example `r chapnum`.1.7. Actuarial Exam Question.** 
...
**Example  `r chapnum`.1.8. Actuarial Exam Question (modified).**
...
**Example `r chapnum`.1.10. Property Fund.** 
...
**Example `r chapnum`.1.11. Actuarial Exam Question.** 
You are given: 
...
`r HideExample('4.1.11', 'Show Example Solution')`
...
Particularly for insurance applications, this is also known as a *credible interval* to distinguish it from credibility theory introduced in Chapter \@ref(C:Credibility).
