...
Figure \@ref(fig:KScvFig) where horizontal axis is Fold=1. This process was repeated for the other seven folds. The results summarized in Figure \@ref(fig:KScvFig) show that the Pareto is a consistently provides a more reliable predictive distribution than the gamma.
...
# Plot the statistics
matplot(1:k,t(cvalvec),type="b", col=c(1,3), lty=1:2, ylim=c(0,0.4), pch = 0, xlab="Fold", ylab="KS Statistic")
...
Even though we present it as a special case of cross-validation, it is helpful to given an explicit definition. Consider a generic statistic $\widehat{\theta}=t(\boldsymbol{x})$ that is an estimator for a parameter of interest $\theta$. The idea of jackknife is to compute $n$ values $\widehat{\theta}_{-i}=t(\boldsymbol{x}_{-i})$, where $\boldsymbol{x}_{-i}$ is the subsample of $\boldsymbol{x}$ with the $i$-th value removed. The average of these values is denoted a
$$
...
##### Table 6.7. Jackkife Estimates of LER at Selected Deductibles {-}
...
- create a bootstrap sample with re-sampling (with replacement) $n$ indices in $\{1,\cdots,n\}$ : let $\mathcal{I}_b$ denote that set of indices. That will be our *training sample*
