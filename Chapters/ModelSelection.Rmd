...
Similarly, with **left-censoring**, we record the *larger* of a variable of interest and a censoring variable. If $C_L$ is used to represent the censoring amount, we record $X_L^{\ast}= \max(X, C_L)$ along with the censoring indicator $\delta_L= \mathrm{I}(X \geq C_L)$. 
...
To summarize, for outcome $X$ and constants $C_L$ and $C_U$, 
  ...
  -------------------- ---------------------------- ------------------------------------
  right censoring       $X_U^{\ast}= \min(X, C_U)$   $\delta_U= \mathrm{I}(X \leq C_U)$
  left censoring        $X_L^{\ast}= \max(X, C_L)$   $\delta_L= \mathrm{I}(X \geq C_L)$
  interval censoring                            
...
For simplicity, we assume non-random censoring amounts and a continuous outcome $X$. To begin, consider the case of right-censored data where we record $X_U^{\ast}= \min(X, C_U)$ and censoring indicator $\delta= \mathrm{I}(X \leq C_U)$. If censoring occurs so that $\delta=0$, then $X \geq C_U$ and the likelihood is $\Pr(X \geq C_U) = 1-F(C_U)$. If censoring does not occur so that $\delta=1$, then $X < C_U$ and the likelihood is $f(x)$. Summarizing, we have the likelihood of a single observation as 
