...
The idea is that the estimator $f_n(x)$ in equation \@ref(eq:KDF) is the average over $n$ `r Gloss('iid')` realizations of a random variable with mean
...
More generally, define the **kernel density estimator** of the `r Gloss('pdf')` at *x* as 
...
$$
using notation $\Gamma(x+1) = x!$ for integer $x$. Assume that $\lambda$ is a draw from a gamma distribution with fixed parameters, say, $\alpha$ and $\theta$, so this has `r Gloss('pdf')`
$$
...
d\lambda .
$$
From Appendix Chapter \@ref(C:AppB) on iterated expectations, we have that the `r Gloss('pmf')` of $X$ can be computed in an iterated fashion as
