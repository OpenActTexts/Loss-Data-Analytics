...
\(X_1, \ldots, X_n\) that are \emph{identically and independently
distributed (iid)} draws from an unknown underlying population
...
\textbf{Example 4.1.10. SOA Exam Question.} You are given:
...
(\emph{BIC}); they are widely cited because they can be readily
...
\(\delta_U= \mathrm{I}(X \leq C_U)\) be a binary variable that is 0 if
censoring occurs and 1 if it does not.
...
along with the censoring indicator \(\delta_L= \mathrm{I}(X \geq C_L)\).
...
\(\delta_U= \mathrm{I}(X \leq C_U)\)\tabularnewline
left censoring & \(X_L^{\ast}= \max(X, C_L)\) &
\(\delta_L= \mathrm{I}(X \geq C_L)\)\tabularnewline
...
\(\delta= \mathrm{I}(X \leq C_U)\). If censoring occurs so that
\(\delta=0\), then \(X \geq C_U\) and the likelihood is
...
\textbf{Example 4.3.7. SOA Exam Question. - Continued.}
...
\textbf{Example 4.4.1. SOA Exam Question.} You are given:
...
\textbf{Example 4.4.5. SOA Exam Question.} You are given:
...
line that is known as the \emph{line of equality}. The area between the
Lorenz curve and the line of equality is a measure of the discrepancy
between the income and population distributions. Two times this area is
known as the \textbf{Gini index}, introduced by Corrado Gini in 1912.
