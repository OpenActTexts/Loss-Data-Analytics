...
In Example 9.3.4, it was shown that $Z=\mathrm{Var}(\mathrm{E}(\bar{X}|\theta))/\mathrm{Var}(\bar{X})$ where $\bar{X}$ is the average loss for $n$ observations.  In equation \@ref(eq:bscred) the $\bar{X}$ is the average loss for $m$ exposures and the same $Z$ formula can be used:
...
This formula is slightly different from Chapter \@ref(C:ModelSelection) because parameter $\beta$ is multiplied by $\lambda$ in the exponential of the gamma *pdf* whereas in Chapter \@ref(C:ModelSelection) $\lambda$ is divided by parameter $\theta$.  We have chosen this form for the exponential to simplify the equation for the expected number of claims. 
