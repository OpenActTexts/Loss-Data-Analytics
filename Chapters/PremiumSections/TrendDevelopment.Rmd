...
This summarization is sometimes known as the *calendar year* method of aggregation to serve as a contrast to the `r Gloss('policy year')` method. In the latter method, all policies start at the beginning of the year. This method is useful for ratemaking methods based on individual contracts and we do not pursue this further here.
...
Broadly speaking, the terms `r Gloss('loss')` and `r Gloss('claim')` refer the amount of compensation paid or potentially payable to the claimant under the terms of the insurance policy. Definitions can vary:
